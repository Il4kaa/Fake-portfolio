        ...
        <header className="fixed top-0 w-full z-50 bg-neutral-950/10 backdrop-blur-xl border-b border-white/10">
          <div className="mx-auto px-6 md:px-20 flex justify-between items-center py-6 md:py-10">
            ...
            <nav className={`fixed md:static inset-0 h-screen md:h-auto w-full md:w-auto bg-neutral-950 md:bg-transparent flex flex-col md:flex-row items-center justify-center md:justify-end gap-10 md:gap-24 duration-500 ease-in-out transition-all ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0 md:translate-x-0 md:opacity-100'}`}>
              <a onClick={() => setIsOpen(false)} href="#about" className="text-2xl md:text-base text-neutral-300 font-mono shadow-2xl hover:text-blue-500 duration-500 transition">Обо мне</a>
              <a onClick={() => setIsOpen(false)} href="#skills" className="text-2xl md:text-base text-neutral-300 font-mono shadow-2xl hover:text-blue-500 duration-500 transition">Навыки</a>
              <a onClick={() => setIsOpen(false)} href="#testimonials" className="text-2xl md:text-base text-neutral-300 font-mono shadow-2xl hover:text-blue-500 duration-500 transition">Отзывы</a>
              <a onClick={() => setIsOpen(false)} href="#projects" className="text-2xl md:text-base text-neutral-300 font-mono shadow-2xl hover:text-blue-500 duration-500 transition">Проекты</a>
              <a onClick={() => setIsOpen(false)} href="#contact" className="text-2xl md:text-base text-neutral-300 font-mono shadow-2xl hover:text-blue-500 duration-500 transition">Контакты</a>
            </nav>
